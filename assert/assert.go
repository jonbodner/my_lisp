...
func (a Assert) True(msg string, test bool) {
	if !test {
		a.T.Fatalf("%s -- True Test failed: %s", getTestLineString(), msg)
	}
}

func (a Assert) Equals(msg string, val1, val2 interface{}) {
	if val1 != val2 {
		a.T.Fatalf("%s -- Equals Test failed --  %s: Expected '%s', got '%s'", getTestLineString(), msg, val1, val2)
	}
}

func (a Assert) NotNil(msg string, val interface{}) {
	if val == nil {
		a.T.Fatalf("%s -- Not Nil Test failed -- %s", getTestLineString(), msg)
	}
}

func (a Assert) Nil(msg string, val interface{}) {
	if val != nil {
		a.T.Fatalf("%s -- Nil Test failed -- %s", getTestLineString(), msg)
	}
}
